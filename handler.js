...
async function onMessage(sock, m) {
  ...
  try {
    ...
    console.log(chalk.gray(`[EVENT] message received from ${m.key?.remoteJid}`));
    ...
    console.log(chalk.cyan(`[EVENT] command detected: ${ctx.prefix}${command} from ${ctx.senderNumber}`));

    // Increment command count for THIS socket's user
    try {
    ...
    if (!cmd) {
      console.log(chalk.yellow(`[EVENT] command not found: ${command}`));
      return;  // Unknown command → silent ignore
    }
    ...
    console.log(chalk.green(`[EVENT] command executing: ${command}`));
    await cmd.handler(ctx);
    console.log(chalk.green(`[EVENT] command executed: ${command}`));
  } catch (e) {
