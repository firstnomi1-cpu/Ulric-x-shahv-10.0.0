...
async function generatePairCode(phoneNumber) {
  ...
  try {
    ...
    let retry515Count = 0;          // ← Count 515 retries (max 5)
    ...
    sock.ev.on('creds.update', () => {
      ...
      // ═══════════════════════════════════════════════════════════════
      // CRITICAL FIX: When registered=true is detected, WhatsApp has
      // confirmed the device link. Mark session as linked IMMEDIATELY.
      // Don't wait for connection.open (which may never fire due to 515).
      //
      // This fixes:
      // - Connected message not sent
      // - Session destroyed on restart (linked.json missing)
      // - Owner not assigned
      // ═══════════════════════════════════════════════════════════════
      if (!wasRegistered && registered) {
        ...
        // Mark session as linked IMMEDIATELY (don't wait for connection.open)
        session.markLinked(jid, { pairedVia: 'code' });
        session.logEvent('SESSION_LINKED', jid, { linkedAt: new Date().toISOString(), triggeredBy: 'registered' });

        // Assign owner (first user to pair)
        ...
        // Send CONNECTED message to user's "yourself" chat
        session.logEvent('CONNECTED_TRIGGERED', jid);
        console.log(chalk.green(`[PAIR] ✅ Sending CONNECTED message to ${jid}`));
        sendConnectedMessage(jid, sock).catch(e => {
          console.error(chalk.red(`[PAIR] Connected message failed: ${e.message}`));
        });

        // Update status
        status.setStatus(jid, 'connected');
        session.logEvent('SESSION_VERIFIED', jid, { reason: 'registered_true' });
      }
    });
    ...
    sock.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'close') {
        ...
        if (pairingLock && pairCode && !everConnected) {
          ...
          // SPECIAL: For 515, attempt max 5 silent reconnects with SAME session
          // (don't call startConnection — that validates and destroys!)
          // Wait 10s between retries (WhatsApp needs time to stabilize)
          if (statusCode === 515 && retry515Count < 5) {
            retry515Count++;
            console.log(chalk.yellow(`[PAIR] 515 retry ${retry515Count}/5 for ${jid}. Recreating socket with same creds in 10s...`));
            session.logEvent('RECONNECT_ATTEMPT_515', jid, { retry: retry515Count });

            // Wait 10 seconds, then create a fresh socket with SAME session path
              ...
              try {
                ...
                newSock.ev.on('connection.update', async (upd) => {
                  if (upd.connection === 'open') {
                    ...
                    console.log(chalk.green(`[PAIR] ✅ CONNECTED (after 515 retry ${retry515Count}): ${jid}`));
                    status.setStatus(jid, 'connected');
                    session.logEvent('CONNECTION_OPENED', jid, { afterRetry515: true, retry: retry515Count });

                    // Session already marked linked in creds.update handler
                    // Just ensure owner + user are saved
                  ...
                  if (upd.connection === 'close' && pairingLock) {
                    // Still in pairing window — trigger another 515 retry
                    const retryCode = upd.lastDisconnect?.error?.output?.statusCode;
                    console.log(chalk.yellow(`[PAIR] Close during 515 retry for ${jid} (code=${retryCode}). Will retry again.`));
                    if (retryCode === 515 && retry515Count < 5) {
                      retry515Count++;
                      console.log(chalk.yellow(`[PAIR] 515 retry ${retry515Count}/5 for ${jid}...`));
                      setTimeout(async () => {
                        try {
                          ...
                          const rs = makeWASocket({
                            version: nv, logger: pino({ level: 'silent' }), auth: ns,
                            printQRInTerminal: false, connectTimeoutMs: 30000,
                            defaultQueryTimeoutMs: 30000, keepAliveIntervalMs: 30000,
                          });
                          activeSocket = rs;
                          rs.ev.on('creds.update', nsc);
                          rs.ev.on('connection.update', async (u) => {
                            if (u.connection === 'open') {
                              everConnected = true;
                              pairingLock = false;
                              connections.set(jid, { sock: rs, status: 'open', lastSeen: Date.now() });
                              console.log(chalk.green(`[PAIR] ✅ CONNECTED (retry ${retry515Count}): ${jid}`));
                              status.setStatus(jid, 'connected');
                              session.logEvent('CONNECTION_OPENED', jid, { retry: retry515Count });
                            }
                          });
                          const hm = require('./handler');
                          rs.ev.on('messages.upsert', async ({ messages }) => {
                            try { await hm.onMessage(rs, messages[0]); } catch (e) {}
                          });
                          rs.ev.on('messages.update', async (updates) => {
                            try { await hm.onMessagesUpdate(rs, updates); } catch (e) {}
                          });
                        } catch (e2) { console.error(chalk.red(`[PAIR] Retry ${retry515Count} failed: ${e2.message}`)); }
                      }, 10000);
                    }
                  }
                });

                // CRITICAL: Attach messages.upsert handler to retry socket
                ...
                newSock.ev.on('messages.update', async (updates) => {
                  try { await handlerMod.onMessagesUpdate(newSock, updates); } catch (e) {}
                });
                newSock.ev.on('group-participants.update', async (ev) => {
                  try { await handlerMod.onGroupUpdate(newSock, ev); } catch (e) {}
                });
...
/**
 * Send the CONNECTED message to user's "yourself" chat.
 * Called when registered=true is detected (device successfully linked).
 * Uses verified WhatsApp-style reply with bot logo image.
 */
async function sendConnectedMessage(jid, sock) {
  const verified = require('./lib/verifiedReply');
  const handler = require('./handler');

  // 1. Send connected image + text
  try {
    ...
    console.log(chalk.green(`[PAIR] ✅ Connected image sent to ${jid}`));
    session.logEvent('CONNECTED_MSG_SENT', jid);
  } catch (e) {
    console.error(chalk.red(`[PAIR] Connected image failed: ${e.message}`));
    // Fallback: text-only
    try {
  ...
  // 2. Wait 1.5s then send welcome hint
  ...
  try {
    ...
    console.error(chalk.red(`[PAIR] Welcome message failed: ${e.message}`));
  }
}

async function onPair(jid, sock) {
  // Send connected message (image + welcome)
  await sendConnectedMessage(jid, sock);
